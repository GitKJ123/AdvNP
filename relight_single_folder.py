...
class RelightNet(object):
    ...
    def build_model(self):
        self.input_luminance_ph = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.img_height, self.img_width, 1], name='input_luminance')
        self.target_lighting_ph = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.SH_dim], name='SH_target')
        self.epoch_ph = tf.compat.v1.placeholder(tf.float32, [], name='epoch_num')
        ...
        self.input_luminance_128 = tf.compat.v1.image.resize_nearest_neighbor(self.input_luminance_ph, (128, 128))
    ...
    def relighting_UNET(self, input_luminance, target_lighting, is_training, epoch):
        ...
        h5_out = tf.compat.v1.image.resize_nearest_neighbor(h5_out, (32, 32))
        ...
        h6_out = tf.compat.v1.image.resize_nearest_neighbor(h6_out, (64, 64))
        ...
        h7_out = tf.compat.v1.image.resize_nearest_neighbor(h7_out, (128, 128))
        ...
        h8_out = tf.compat.v1.image.resize_nearest_neighbor(h8_out, (256, 256))
    ...
    def discriminator_256(self, images, is_training, reuse = False):
        with tf.compat.v1.variable_scope("discriminator_256") as scope:
            if(reuse): 
    ...
    def discriminator_128(self, images, is_training, reuse = False):
        with tf.compat.v1.variable_scope("discriminator_128") as scope:
            if(reuse): 
                tf.compat.v1.get_variable_scope().reuse_variables()
            else:
                assert tf.compat.v1.get_variable_scope().reuse == False
    ...
    def conv2d(self, input_, output_dim, kernel = (3,3), strides=(2, 2), use_bias=True, stddev=0.02, name="conv2d"):
        with tf.compat.v1.variable_scope(name):
            w = tf.compat.v1.get_variable('w', [kernel[0], kernel[1], input_.get_shape()[-1], output_dim], initializer= tf.compat.v1.truncated_normal_initializer(stddev=stddev))
    ...
    def deconv2d(self, input_, output_shape, kernel=(3,3), strides=(2,2), stddev=0.02, use_bias = True, name="deconv2d", with_w=False, reuse = False):
        with tf.compat.v1.variable_scope(name):
            ...
            deconv = tf.compat.v1.layers.conv2d_transpose(input_, output_shape, kernel, strides, padding='SAME', data_format='channels_last', activation=None, use_bias=use_bias,
    ...
    def linear(self, input_, output_size, scope="Linear", stddev=0.02, bias_start=0.0, relu=True):
        ...
        with tf.compat.v1.variable_scope(scope or "Linear"):
            matrix = tf.compat.v1.get_variable("Matrix", [input_.shape[1].value, output_size], tf.float32,
    ...
    def test(self):
        ...
        self.saver = tf.compat.v1.train.Saver(keep_checkpoint_every_n_hours=1, max_to_keep = 1000)
        ...
        for file in img_files:
            print(file)
            curr_RGB_image = imageio.imread(source + file)
            ...
            # convert RGB to YUV
            curr_input_luminance = np.reshape(0.899*curr_RGB_image[:, :, 0]+0.587*curr_RGB_image[:, :, 1]+0.114*curr_RGB_image[:, :, 2], (256, 256, 1))
